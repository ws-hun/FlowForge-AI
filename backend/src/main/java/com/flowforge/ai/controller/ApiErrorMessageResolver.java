...
final class ApiErrorMessageResolver {
    ...
    private static final Map<String, String> EXACT_MESSAGES = Map.ofEntries(
            ...
            Map.entry("Source Prompt not found", "找不到来源 Prompt"),
            Map.entry("Source Task not found", "找不到来源运行"),
            Map.entry("Source Flow node not found", "找不到来源 Flow 节点"),
            Map.entry("Source Flow revision not found", "找不到来源 Flow 修订"),
            Map.entry("Flow source must be either Prompt or Flow", "Flow 只能保留一种来源"),
            Map.entry("Source Prompt must be included as a Prompt node", "来源 Prompt 必须作为 Prompt 节点加入 Flow"),
            Map.entry("sourcePromptId is required when sourceTaskId is provided", "从运行创建 Flow 时必须提供来源 Prompt"),
            Map.entry("Failed Task cannot be used as a Flow source", "失败运行不能用于创建 Flow"),
            Map.entry("Source Prompt does not belong to the source Task", "来源 Prompt 与运行记录不匹配"),
