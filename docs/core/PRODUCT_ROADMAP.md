...
- Run comparison also verifies whether two immutable v5 plans declared the same ordered Provider fan-in. Missing legacy declarations remain explicitly unavailable and are never reconstructed from the current Flow.
- Run comparison now separates input, Provider fan-in, and execution evidence. Provider/model/status differences are actionable, while missing legacy call or Attempt fields remain unknown instead of being reported as changes.
...
- Flow previews and immutable run traces now share `flow-plan-v5`, a deterministic linear node plan that records node responsibilities, independently addressable artifact contracts, `compiled-reference` input resolution, and the single AI Task Provider boundary while keeping legacy plans readable.
- The AI Task plan now declares an ordered Provider fan-in from the immutable Flow objective and every preceding Input / Prompt contribution. This remains an inspectable single-pass contract, not persisted-artifact loading or independent node execution.
- New v5 runs persist that ordered fan-in as metadata-only Provider input references owned by the AI Task Artifact. Run detail can inspect and navigate those declarations without preloading payloads or inventing references for legacy history.
- Artifact detail validates persisted Provider input order, ownership, objective position, source identity, state, and fingerprint before exposing the metadata-only declaration.
- PostgreSQL migration V11 adds immutable database checks for Provider input identity, storage combinations, source fields, and lowercase SHA-256 fingerprints; relational same-run lineage remains a service-level rule.
- Flow previews and immutable traces now carry `flow-input-resolution-v1`, making `compiled-reference` the active mode and reserving `persisted-artifact` for the future node-sequential runtime without changing current single-pass execution.
...
- Flow run trace inspection invalidates artifact and lineage caches when the selected run changes, and ignores late responses from an older run so reused node keys cannot display stale execution evidence.
- Flow execution preview keeps the last known compiled input visible during refresh failures while exposing the current failure and retry action, so a temporary preview outage cannot be mistaken for a valid current execution context.
- Flow execution preview now acquires an exclusive preparation gate before saving pending editor changes; repeated refresh actions cannot trigger concurrent Flow revisions, and a preparation completing after Flow navigation cannot start a stale preview request.
- Direct Flow execution now treats pending-edit persistence and Provider invocation as one exclusive user action. It captures the source Flow identity, node snapshot, Run Brief, and variables before the call, rejects duplicate Provider execution, and never applies a late completion to a different active Flow.
- Flow Space retains ownership of a direct execution through preparation, Provider invocation, and run-history reconciliation. Navigation and browser unload now respect that full boundary so pending edits cannot be saved twice and completion state cannot be abandoned mid-reconciliation.
- The workspace execution lock records the active action and immutable source identity. History can identify the one run being replayed or recovered without making every timeline action appear active.
- Flow Space separates local run ownership from global Provider occupancy: its own execute, replay, and recovery actions protect Flow identity through reconciliation, while a run started elsewhere only blocks another Provider call and does not freeze Flow exploration.
- Active execution identity remains visible across navigation: the global shell and AI Command distinguish direct tasks, Flow runs, historical reruns, and recoveries, and the shell status links back to the immutable execution source without masking an offline application state.
- Historical reruns and failed-run recoveries no longer clear or replace unfinished AI Command work; their returned Result remains owned by the History or Flow surface that initiated the action.
- Direct AI Command execution captures one immutable input/source/variable snapshot. Edits made while the Provider is running remain the next draft instead of being cleared by the earlier run, and failure provenance cannot drift to a newly selected source.
- Historical execution actions return an explicit immutable run outcome for both success and persisted Provider failure. History and Flow Space can open that exact run without borrowing AI Command result or failure state.
- AI Command Result reuse is owned by the completed Result rather than the next editable draft. Starting a continuation or input variant marks a historical preview honestly, while edits made during a real execution do not hide the new Result's reuse actions.
- AI Command keeps its composer editable during Provider work while naming the boundary honestly: the submitted snapshot is immutable, and any subsequent edit is visibly retained as the next draft.
- Result-to-Prompt promotion is serialized per immutable run. Repeated actions from the same Result share one request and cannot create duplicate Prompt assets.
- Result promotion progress is scoped to its source run. History keeps unrelated Results reusable while one Prompt is being created, and AI Command only marks its own Result as busy.
- Result-to-Flow conversion serializes the complete Prompt-and-Flow creation chain per immutable run, so repeated entry points cannot produce duplicate Flow assets after sharing the same Prompt request.
- Result-to-Flow progress is scoped to the source run, and overlapping Flow persistence uses reference-counted loading state. One conversion no longer makes every Result appear active or clears shared progress before another Flow request finishes.
- Task-sourced Prompt promotion is idempotent across refreshes, tabs, and clients. The backend serializes promotion on the immutable source Task and returns the earliest existing Prompt without deleting legacy duplicates.
- Result-to-Flow promotion is also idempotent across refreshes, tabs, and clients. The backend locks the immutable source Task, verifies that the intermediate Prompt belongs to that Result, and returns the earliest existing promoted Flow; direct Prompt Library reuse remains intentionally repeatable.
- Every new Flow run snapshot preserves the Flow's Result, Prompt, or Flow creation origin as immutable execution context. History and Flow Space can reopen those real sources without reconstructing lineage for legacy runs.
- Run comparison now evaluates saved Flow identity and creation origin independently, so two different Flow assets can still be recognized as sharing one Result, Prompt, or Flow starting point while incomplete legacy evidence remains unavailable.
- Run comparison panels can navigate directly to the executed Flow or its saved Result, Prompt, or Flow origin. Navigation uses immutable snapshot IDs and preserves the selected run deep link for continued inspection.
- Run comparison panels are also creation entry points: each successful side can save its immutable Result as a Prompt or create a Flow, while failed runs stay evidence-only. Scoped loading and existing Prompt detection keep the comparison surface calm and prevent duplicate promotion requests.
- Run comparison explains the saved Flow context at field level: title, objective, runtime brief, variables, node membership, node content, and shared-node order. It compares only the two immutable run snapshots, so current editor changes cannot rewrite historical evidence.
- Either side of a run comparison can branch its immutable Flow snapshot into an independent Flow and seed the saved runtime brief. Successful runs keep separate Result and snapshot reuse choices; failed runs expose only snapshot branching so invalid output is never promoted as a reusable Prompt.
- Node-level snapshot changes in the comparison surface can open the affected saved Flow node directly, preserving `flow`, `node`, and `run` deep-link context for inspection. Metadata and variable changes remain explanatory and do not add noisy actions.
...
- Failed Flow recovery now has an explicit `POST /api/tasks/{id}/recover` identity. History and Flow Space distinguish `recoveryOfTaskId` / `stored-input-recovery` from ordinary successful reruns while retaining the exact failed input and snapshot.
...
- Modern `flow-plan-v5` records are shape-validated before trace and artifact persistence: linear step order, node responsibility, Provider boundary, artifact chain, and dependency declarations must agree; this protects the single-pass contract without enabling sequential execution.
- Flow Space and the run trace preserve the same single-pass node state matrix: Input / Prompt are prepared contributions, AI Task is the Provider terminal, and Output is completed only after a successful call or skipped after failure.
...
- Provider response parsing tolerates BOMs, fenced JSON, short explanatory prose, nested braces inside JSON strings, and array-based chat message content; when no valid structured payload can be recovered, the readable model text and complete raw response remain available.
...
- 全局搜索在无 Prompt 缓存且读取失败时提供就地重试动作，保留当前关键词和其他创建入口，避免用户为了恢复复用链路离开当前工作。
...
- Flow 运行深链会等待当前 Flow 的运行记录完成确认，不再被旧的全局 History 缓存提前判定为不存在；读取失败时保留 `run` 参数，重试后继续打开原运行。
...
- Provider Vault 在刷新失败时继续展示当前已载入配置，并提供列表级重试；密钥管理不会因一次读取故障退化为空状态。
