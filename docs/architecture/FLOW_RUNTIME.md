...
The immutable Flow snapshot also records the Flow asset's creation origin at execution time. A Result-derived Flow carries its source Task summary and intermediate Prompt identity, a Prompt-derived Flow carries its source Prompt identity, and a Flow revision branch carries its source Flow/version identity. These fields explain where the executed asset came from; they do not change Provider input compilation or node execution semantics. Legacy snapshots keep absent origin fields null.
...
Migration `V11__tighten_flow_provider_input_reference_contract.sql` adds database checks for non-empty reference identity, the `flow:objective` / `flow-snapshot` pairing, node-backed source fields, and lowercase 64-character hexadecimal fingerprints. The database constraint is a first line of defense; same-run ownership, source order, and fingerprint equality remain service-policy checks because they require joining the referenced Artifact.
...
Legacy traces without an execution plan remain valid and deserialize with `executionPlan = null`. The frontend labels these records as legacy instead of synthesizing a plan that did not exist at run time. Missing `executionMode` or `providerCallCount` evidence is also shown as not recorded rather than being defaulted to the current single-pass contract.
...
When a saved snapshot diff identifies a node addition, removal, content change, or reorder, the comparison UI retains the immutable node ID and opens the corresponding side's Flow with the run deep link. The navigation does not rewrite the snapshot or infer missing legacy node identity; metadata-only changes deliberately have no node target.
