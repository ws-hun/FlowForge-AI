...
`FlowNodeArtifactService` owns atomic artifact materialization and validates that every current upstream reference resolves to an earlier planned output. It also binds each v5 Provider input declaration to the persisted AI Task Artifact without copying payloads. `FlowNodeArtifactQueryService` exposes ordered metadata, one addressable payload and its declared Provider inputs at a time, and a metadata-only lineage path with explicit termination states. Successful runs write Task and artifacts in the execution transaction. `TaskFailureRecorder` writes failed Task, artifact states, input references, and Provider Attempt together in one `REQUIRES_NEW` transaction so partial failure history cannot survive.
