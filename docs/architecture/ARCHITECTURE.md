...
The only current input resolution is `compiled-reference`: lineage describes how the immutable snapshot contributed to one compiled Provider request. The runtime does not load each predecessor payload from PostgreSQL and still performs exactly one Provider call. Node-level provenance therefore makes the existing boundary inspectable but does not activate `node-sequential` execution.
